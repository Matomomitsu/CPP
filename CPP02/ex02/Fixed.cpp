...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/05/03 19:37:55 by mtomomit          #+#    #+#             */
...
Fixed	Fixed::operator*(Fixed const &rhs) const{
	...
	product = static_cast<int64_t>(getRawBits()) * static_cast<int64_t>(rhs.getRawBits());
	temp.setRawBits((product + (1 << (_fractionalBits - 1))) >> _fractionalBits);
	return (temp);
}
...
Fixed	Fixed::operator/(Fixed const &rhs) const{
	...
	dividend = static_cast<int64_t>(getRawBits()) << _fractionalBits;
	quotient = static_cast<int32_t>((dividend + (rhs.getRawBits() / 2)) / rhs.getRawBits());
	temp.setRawBits(quotient);
	return (temp);
}
